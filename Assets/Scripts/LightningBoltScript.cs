...
namespace DigitalRuby.LightningBolt
{
	...
	public class LightningBoltScript : MonoBehaviour
	{
		...
		private void Update()
		{
			if (windup && EndObject != null)
			{
				transform.position = new Vector3(EndObject.transform.position.x, transform.position.y, EndObject.transform.position.z);
			}
			else if (!stop && EndObject != null)
			{
				orthographic = (Camera.main != null && Camera.main.orthographic);
				if (timer <= 0.0f)
				{
		...
		public void Trigger()
		{
			...
			startIndex = 0;
			GenerateLightningBolt(StartPosition, EndPosition, Generations, Generations, 0.0f);
			UpdateLineRenderer();
		}
